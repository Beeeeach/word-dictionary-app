...
import { validateNoBannedWords } from "@/lib/moderation/content-safety";
...
export async function updateDisplayName(
  ...
  // 荒らし防止: なりすまし・侮辱等を含む表示名を拒否する
  const bannedWordError = validateNoBannedWords(displayName);
  if (bannedWordError) {
...
export async function updateBio(
  ...
  const bannedWordError = validateNoBannedWords(bio);
  if (bannedWordError) {
    return { error: bannedWordError };
  }
...
/**
 * 英語学習者モードのオン/オフを切り替える。
 * オンにすると:
 *  - フィード等に英語投稿(単語がアルファベットのみ)のみが表示される
 *  - アプリ全体のUIが英語表示になる
 * 端末をまたいで同じ設定になるよう、users.learner_mode に保存する。
 */
export async function toggleLearnerMode(
  _prevState: UpdateProfileResult,
  formData: FormData
): Promise<UpdateProfileResult> {
  const supabase = await createClient();
  ...
  const enabled = formData.get("learner_mode") === "true";

  const { error } = await supabase
    .from("users")
    .update({ learner_mode: enabled })
    .eq("id", user.id);
