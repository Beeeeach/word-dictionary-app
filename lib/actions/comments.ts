 ...
 *  - 直近1分間のコメント数が3件を超えていないか(レート制限)
 *  - 直近1分間に全く同じ内容をコメントしていないか(連打防止)
 *  - 禁止ワードを含んでいないか
 */
export async function addComment(
  ...
  const bannedWordError = validateNoBannedWords(body);
  if (bannedWordError) {
    return { error: bannedWordError };
  }

  const rateLimitError = await checkRateLimit(supabase, user.id, "comments");
  if (rateLimitError) {
    return { error: rateLimitError };
  }

  const duplicateError = await checkDuplicateContent(supabase, user.id, "comments", body);
  if (duplicateError) {
