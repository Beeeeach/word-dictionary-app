 ...
 * 企画: 学習者モードがオンの間は英語の投稿しか見えないようにする。
 * DBのcollation依存を避けるため、フィルタはアプリ側(JS)で行う。
 */
 ...
 * is_hiddenはDBクエリ側でも絞れるが、RPC経由の取得など複数の経路があるため、
 * 取得後にアプリ側で一貫してフィルタする形にしている。
 */
 ...
 * また、3件以上通報され自動非表示(is_hidden)になった投稿、および
 * 自分がブロックしたユーザーの投稿は常に除外する。
 */
export async function getFeedPosts(
  ...
  if (sort === "following") {
    ...
    const notHiddenOrBlocked = filterHiddenAndBlocked(
      data as unknown as { user_id: string; is_hidden?: boolean; word: string }[],
      blockedUserIds
    );
    const filtered = filterEnglishOnly(notHiddenOrBlocked, learnerMode);
    const posts = await attachReactionData(filtered, currentUserId);
  ...
  if (currentUserId) {
    ...
    if (!rpcError && rankedIds) {
      ...
      if (!error && data) {
        ...
        const notHiddenOrBlocked = filterHiddenAndBlocked(
          orderedRaw as unknown as { user_id: string; is_hidden?: boolean; word: string }[],
          blockedUserIds
        );
        const filtered = filterEnglishOnly(notHiddenOrBlocked, learnerMode);
        const posts = await attachReactionData(filtered, currentUserId);
 ...
 * 学習者モードによるフィルタは行わない（詳細ページは直接リンクされたら
 * 日本語投稿でも表示できるようにするため）。
 * is_hiddenの投稿も直リンクなら表示する(通報中でも投稿者本人が確認できるようにするため)。
 */
